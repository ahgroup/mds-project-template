...
- `output/`: computed result objects saved by the exploration and analysis code
  (RDS or CSV), before they are turned into finalized figures and tables.
- `figures/`: figures generated by code.
- `tables/`: tables generated by code.
- `large-files/`: generated output files that are too large to track with
  GitHub.
