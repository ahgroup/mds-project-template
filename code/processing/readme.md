...
Each way of doing this is a reasonable approach, pick whichever one you prefer or makes the most sense for your setup. You can also mix and match. For instance for an exploration task, it might make sense to produce a Quarto file. Then I would use the 2nd or 3rd approach. If you do a main analysis, then you might just want to have a script that does the data analysis and saves the results to a file, for later use/processing. You might not need or want a Quarto file for that.

If a project uses another language for part of processing, place that script here too, for example `02-geocode-data.py` or `03-clean-records.jl`. Move to language-specific subfolders only when the project becomes large enough to need that extra organization.
