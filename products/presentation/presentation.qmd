---
title: Example Quarto slides
subtitle: "with some thoughts on how to set up things"
date: 2026-01-01
author: "NAME"
format:
  revealjs:
    theme: default
    css: "./media/my-presentation-styling.css"
    transition: none
    incremental: false
    cap-location: bottom
    embed-resources: true
    slide-number: true
    show-slide-number: all
    auto-stretch: true
    smaller: false
bibliography: ../../assets/dataanalysis-references.bib
csl: ../../assets/american-journal-of-epidemiology.csl
---

## Overview

-   A few simple slides using the [`revealjs` (html) format](https://quarto.org/docs/presentations/revealjs/).
-   For other formats (e.g. PowerPoint, or Beamer/pdf), see [here](https://quarto.org/docs/guide/).


## Figures and tables

The suggestion is to place figures/tables/RDS files etc. in the `media` folder.

You could also pull it from the `results` folder, but the advantage of copying them into `media` is that you have everything related to your presentation in one place, and if content in `results` continues to be changed/updated, you can ensure you have the version you used in your presentation by copying it into `media`.



## Example slide

This shows the summary table. It is pulled in from an RDS file and rendered in an R chunk with the `flextable` package, which is the default table package for this template (see `code-guidelines.md`). Other good options are `gt`, or `knitr::kable()` when a plain, minimal table is all you need.

Note that we could have loaded the data with `here()`, but if we ever want to copy this presentation to another folder outside the current project, you would have to adjust the path.

```{r}
#| label: tbl-summary-table
#| tbl-cap: "Data summary table."
#| echo: FALSE
resulttable=readRDS("./media/summary-table.rds")
flextable::flextable(resulttable) |>
  # scale the table to the page/slide width instead of its natural width,
  # so wide tables do not run off the page
  flextable::set_table_properties(layout = "autofit", width = 1)
```

## Example slide

This shows a figure created by the analysis script. It is inserted using Quarto/Markdown syntax (not knitr code, but that would be possible too).

![](./media/height-weight.png){fig-align="center" width="420"}

## Example slide

This shows the model fitting results as table.

```{r}
#| label: tbl-result-table-2
#| tbl-cap: "Linear model fit table."
#| echo: FALSE
result_table_2 = readRDS("./media/result-table-2.rds")
flextable::flextable(result_table_2) |>
  # scale the table to the page/slide width instead of its natural width,
  # so wide tables do not run off the page
  flextable::set_table_properties(layout = "autofit", width = 1)
```

## Example slide with reference

This paper [@leek2015] discusses types of analyses.

## Further Resources

* [Quarto Presentation Documentation](https://quarto.org/docs/presentations/)
* [Slidecraft 101](https://emilhvitfeldt.com/project/slidecraft-101/) is a nice blog post showing some more advanced things one can do with Quarto slides.

## References
