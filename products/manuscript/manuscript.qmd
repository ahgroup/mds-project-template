...
library(flextable) # for rendering tables, see code-guidelines.md
```


_Placeholder: the author names, affiliations, ORCID, and corresponding-author
email below are template examples, not real people. Replace all of them with
real project information, and delete this note._
...
flextable::flextable(resulttable) |>
  # scale the table to the page/slide width instead of its natural width,
  # so wide tables do not run off the page
...
```{r}
#| label: fig-result
#| fig-cap: "Observed height (cm) versus weight (kg), colored by gender."
#| echo: FALSE
...
#| tbl-cap: "Linear model of height (cm) by weight (kg) and gender; F is the gender reference category."
#| echo: FALSE
result_table_2 <- readRDS(here("results","tables","result-table-2.rds"))
flextable::flextable(result_table_2) |>
  # scale the table to the page/slide width instead of its natural width,
  # so wide tables do not run off the page
  flextable::set_table_properties(layout = "autofit", width = 1)
```
